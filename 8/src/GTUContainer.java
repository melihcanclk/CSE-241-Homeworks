...
public abstract class GTUContainer<T> {
    ...
    protected T[] arr;
    /**
     * Class type of any GTUContainer<T> class (protected)
     */

    protected Class<T> _type;
    /**
     * Size of any GTUContainer<T> class that can take values (protected)
     */

    protected int _size;
    /**
     * Capacity any GTUContainer<T> class that increases 2^n (protected)
     */
    ...
    public GTUContainer(final Class<T> type) {
    ...
    class GTUIteratorContainer implements GTUIterator<T> {
        /**
         * Position representation of GTUIterator (protected)
         */
    ...
    public void insert(final T inserted) {
        ...
        try {
            ...
            if (_size + 1 >= capacity) {
                ...
                final T[] dest = (T[]) java.lang.reflect.Array.newInstance(_type, capacity);
    ...
    public void erase(final GTUIterator<T> iter) {
        boolean isAlreadyInserted = false;
        for (final GTUIterator<T> iter_temp = this.iterator(); iter_temp.hasNext(); iter_temp.next()) {
            if (iter_temp.current() == iter.current()) {
                for (final GTUIterator<T> iter2 = iter_temp; iter2.hasNext(); iter2.next()) {
